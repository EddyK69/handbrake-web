Use the mqtt branch before building, so the images contain the code and 0.8.2 version from that branch:
```
...
Build both images from the repository root. The root context is required because the Dockerfiles copy files from client, shared, and the workspace root.
```
...
podman build `
  --pull-always `
  --build-arg HANDBRAKE_BUILD_TAG=latest `
